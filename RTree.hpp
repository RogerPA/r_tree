...
class RTree {
 public:
  ...
  struct Node {
    ...
    std::shared_ptr<typename RTree<N, ElemType, M, m>::Node> split_node(std::shared_ptr<Node> &l, const SpatialObject &g);

    std::pair<size_t, size_t> pick_seed(std::vector<SpatialObject> &Entry1);

    std::pair<size_t, bool> pick_next(std::vector<SpatialObject> &Entries,std::pair<Rectangle<N>, Rectangle<N>> &Rectang);
...
template <size_t N, typename ElemType, size_t M, size_t m>
std::pair<size_t,size_t> RTree<N, ElemType, M, m>::Node::pick_seed(std::vector<SpatialObject> &Entry1) {

  float temp = 0;
  float temp2 = 0;

  std::pair<size_t, size_t> seed; 
  seed.first = seed.second = size_t(0);
  ...
  for (int i = 0; i < M + 1; i++) {
    for (int j = i + 1; j < M + 1; j++) {
          ...
          temp2 = abs(jj.get_area() - Entry1[i].box.get_area() - Entry1[j].box.get_area());
...
template <size_t N, typename ElemType, size_t M, size_t m>
std::pair<size_t, bool> RTree<N, ElemType, M, m>::Node::pick_next(std::vector<SpatialObject> &Entries, std::pair<Rectangle<N>,Rectangle<N>> &Rectang) {
  ...
  for (int i = 0; i < Entries.size(); i++) {
      ...
      Rectang.first.adjust(Entries[i].box);
      Rectang.second.adjust(Entries[i].box);

      d1 = abs(Rectang.first.get_area() - Entries[i].box.get_area());
      d2 = abs(Rectang.second.get_area() - Entries[i].box.get_area());

      float temp2 = abs(d1 - d2);
...
//NO ME ALCANZO EL TIEMPO PARA REALIZAR LA FUNCIONALIDAD DEL SPLIT PROFESOR
  ...
  pick_next(l->entry1,l1_rectang,l2_rectang)
