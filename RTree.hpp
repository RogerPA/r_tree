 ...
 public:
  ...
  struct SpatialObject {
    ...
    std::shared_ptr<Node> child_pointer;
     ...
     SpatialObject& operator=(const SpatialObject &newSpatialObj) {

      box = newSpatialObj.box;
      identifier = newSpatialObj.identifier;
      child_pointer = newSpatialObj.child_pointer;
  ...
  struct Node {
    ...
    std::shared_ptr<typename RTree<N, ElemType, M, m>::Node> split_node(std::shared_ptr<Node> &l, std::shared_ptr<Node> &ll, const SpatialObject &g);

    void pick_seed(std::vector<SpatialObject> &Entry1, int &seedI, int &seedJ);

    void pick_next(std::vector<SpatialObject> &Entries,Rectangle<N> &rectang1, Rectangle<N> &rectang2, int &e, int &ng);
...
std::vector<ElemType> &RTree<N, ElemType, M, m>::operator[](const Rectangle<N> &box) {
  /*-----------NO LOGRE IMPLEMENTARLO PROFESOR--------------------*/
}
...
//FUNCIONES DEL SPLIT, pick next, pick seed

template <size_t N, typename ElemType, size_t M, size_t m>
void RTree<N, ElemType, M, m>::Node::pick_seed(std::vector<SpatialObject> &Entry1, int &seedI, int &seedJ) {
  ...
  for (int i = 0; i < M + 1; i++) {
    for (int j = i + 1; j < M + 1; j++) {
          ...
          Rectangle<N> jj = Entry1[i].box;
          jj.adjust(Entry1[j].box);

          temp = abs(jj.get_area() - Entry1[i].box.get_area() - Entry1[j].box.get_area());
...
template <size_t N, typename ElemType, size_t M, size_t m>
void RTree<N, ElemType, M, m>::Node::pick_next(std::vector<SpatialObject> &Entries, Rectangle<N> &rectang1, Rectangle<N> &rectang2, int &e, int &ng) {
  ...
  for (int i = 0; i < Entries.size(); i++) {
      ...
      rectang1.adjust(Entries[i].box);
      rectang2.adjust(Entries[i].box);  
      
      d1 = abs(rectang1.get_area() - Entries[i].box.get_area());
      d2 = abs(rectang2.get_area() - Entries[i].box.get_area());
...
template <size_t N, typename ElemType, size_t M, size_t m>
std::shared_ptr<typename RTree<N, ElemType, M, m>::Node>RTree<N, ElemType, M, m>::Node::split_node(std::shared_ptr<Node> &l, std::shared_ptr<Node> &ll,  const SpatialObject &new_entry) {

/*//NO ME ALCANZO EL TIEMPO PARA REALIZAR LA FUNCIONALIDAD DEL SPLIT PROFESOR
  pick_seed(.....);
  for(int i = 0; i < new_entry)....Asignamos todas las entradas
  pick_next(........);
  con su rectangulo minimo  
  adjust_tree() 
  */
  return nullptr;
}
...
RTree<N, ElemType, M, m>::Node::insert(const SpatialObject &new_entry) {
  ...
  /*else
  {
    ...
    l.reset(this);
    return split_node(l, ll, new_entry);
